...
# classification of positive and negative score, using SentiWordNet
class TweetSentiment(object):
    def __init__(self):
        ...
        with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'data/SentiWordNet_3.0.0_20130122.txt'),
                  'r') as csvfile:
            ...
            for line in reader:
                ...
                for term in SynsetTerms.split(" "):
                    ...
                    sentiment_scores[key].append((float(PosScore), float(
                        NegScore)))  # returns the list of scores for that key [('key', [posscore, negscore]), ...]
                    # we classify all the words by + or -

        # Returns the average of the array elements(scores)
        for key, value in sentiment_scores.items():
            sentiment_scores[key] = np.mean(value, axis=0)

        self.sentimentScores = sentiment_scores

    def word_score(self, word):
        pos = nltk.pos_tag([word])[0][1]
        return self.score(word, pos)

    def tweet_score(self, tweet):
    ...
    def position_vector(self, tweet):
